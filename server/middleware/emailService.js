...
const sendClientConfirmation = async (lead) => {
  await transporter.sendMail({
    from: `"ClearPath Pardons & US Waivers" <${process.env.EMAIL_USER}>`,
    to: lead.email,
    subject: 'We Received Your Application — ClearPath Pardons',
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
        <div style="background:#0B1F3A;padding:24px;text-align:center">
          <h1 style="color:#C9A84C;margin:0;font-size:22px">ClearPath Pardons & US Waivers</h1>
        </div>
        <div style="padding:32px;background:#fff;border:1px solid #e5e7eb">
          ...
          <p style="color:#6b7280;line-height:1.7">Thank you for reaching out. A ClearPath specialist will review your case and contact you within <strong>24 business hours</strong>.</p>
          ...
          <p style="color:#6b7280">In the meantime, if you have urgent questions:<br>
            ...
            ✉️ info@clearpathpardons.ca
          </p>
          <a href="https://clearpathpardons.ca" style="display:inline-block;background:#C9A84C;color:#0B1F3A;padding:12px 24px;text-decoration:none;border-radius:4px;font-weight:bold;margin-top:16px">Visit Our Website</a>
        </div>
        <div style="padding:16px;text-align:center;color:#9ca3af;font-size:12px">
          © 2026 ClearPath Pardons & US Waivers — Mississauga, ON<br>
...
const sendAdminNotification = async (lead) => {
  await transporter.sendMail({
    from: `"ClearPath Website" <${process.env.EMAIL_USER}>`,
